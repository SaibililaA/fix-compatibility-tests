...
#define noptions ( 9 ) 
  ...
  do {
    ...
    {
      printversionandexit ( "This is TANGLE, Version 4.6" , nil , "D.E. Knuth" 
      , nil ) ;
    } 
  ...
  {
    fprintf ( stderr , "%s%s\n",  "tangle" , ": Need one to three file arguments." ) ;
    usage ( "tangle" ) ;
  } 
  webname = extendfilename ( cmdline ( optind ) , "web" ) ;
  if ( optind + 2 <= argc ) 
  {
    if ( strcmp ( chartostring ( '-' ) , cmdline ( optind + 1 ) ) != 0 ) 
    chgname = extendfilename ( cmdline ( optind + 1 ) , "ch" ) ;
  } 
  if ( optind + 3 == argc ) 
  {
    pascalname = extendfilename ( cmdline ( optind + 2 ) , chartostring ( 'p' 
    ) ) ;
	...
	while ( q != 0 ) {
	    
	  {
	    ...
	    while ( ( k < bytestart [q + 3 ]) && ( s < unambiglength ) ) {
	      ...
	      {
		if ( ( strictmode || forceuppercase ) && ( c >= 97 ) && ( c <= 
		122 ) ) 
		c = c - 32 ;
...
sendtheoutput ( void ) 
{
  ...
  while ( stackptr > 0 ) {
	...
	while ( ( k < maxidlength ) && ( j < bytestart [curval + 3 ]) ) {
	  ...
	  j = j + 1 ;
	  if ( forceuppercase && ( outcontrib [k ]>= 97 ) && ( outcontrib [
	  k ]<= 122 ) ) 
	  outcontrib [k ]= outcontrib [k ]- 32 ;
	  else if ( forcelowercase && ( outcontrib [k ]>= 65 ) && ( 
	  outcontrib [k ]<= 90 ) ) 
	  outcontrib [k ]= outcontrib [k ]+ 32 ;
...
zscannumeric ( namepointer p ) 
{
  ...
  while ( true ) {
	...
	do {
	  ...
	  else val = 10 * val + nextcontrol - 48 ;
	  nextcontrol = getnext () ;
	...
	do {
	  ...
	  else val = 16 * val + nextcontrol - 48 ;
	  nextcontrol = getnext () ;
...
scanmodule ( void ) 
{
  ...
  while ( true ) {
    ...
    goto lab30 ;
    nextcontrol = getnext () ;
    if ( ( nextcontrol != 130 ) || ( buffer [idfirst ]== 34 ) ) 
    {
      {
	putc ('\n',  stdout );
...
void mainbody( void ) {
  ...
  buffer [0 ]= 32 ;
  inputhasended = false ;
  Fputs ( stdout ,  "This is TANGLE, Version 4.6" ) ;
  fprintf ( stdout , "%s\n",  versionstring ) ;
  phaseone = true ;
  modulecount = 0 ;
  do {
  ...
  if ( changelimit != 0 ) 
  {
    {register integer for_end; ii = 0 ;for_end = changelimit - 1 ; if ( ii 
    <= for_end) do 
