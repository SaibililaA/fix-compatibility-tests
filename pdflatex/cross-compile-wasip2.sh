...
mkdir -p "$root/wasip2-build" && cd "$root/wasip2-build" && rm -rf ./* ./.??* 2>/dev/null || true
...
"$root/texlive-source/configure" \
	...
	CFLAGS="--target=wasm32-wasip2 --sysroot=$WASI_SYSROOT -O2 -I$root/wasi-compatibility -include $root/wasi-compatibility/wasi-compatibility.h" \
